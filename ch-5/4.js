...
function dominantDirection(text){
    ...
    return scripts.reduce((prev, curr) => {
        //fairly convoluted code, non functional approach would seem cleaner
        //console.log(prev);
        //console.log(curr);
        if(prev.count === curr.count){
        //need to deal with scenario where counts are the same
            return {name:"no dominant direction"};}
        //need a default to deal with case where only 1 writing direction 
        return (prev.count > curr.count) ? prev : curr
    }, {name:'default',count:0}).name;
}
